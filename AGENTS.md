...
- Subentry **titles follow TapHome**: `config_flow.py:device_subentry_title` is the single title formula (device name/zone/category + id, or `name — value` for per-value sensors); `__init__.py:_async_sync_subentry_titles` re-applies it on every setup while the stored `auto_title` still equals the title, so a title the user renamed in HA is never overwritten. It runs before the update listener is registered, so its writes do not trigger a reload.
