...
3. Registers the Core as the hub device (with `configuration_url` to its local `/localapilog` page on local connections), registers the optional push webhook, removes registry entities whose device is no longer configured, and runs new-device detection — at setup **and periodically** (`NEW_DEVICE_SCAN_INTERVAL`, 15 min, via `hub.async_discover_new_devices`). Unknown exposed devices raise a fixable repair issue (`repairs.py:NewDeviceRepairFlow` — pick a platform or ignore); the known-device baseline lives in `entry.data[CONF_KNOWN_DEVICE_IDS]`.
