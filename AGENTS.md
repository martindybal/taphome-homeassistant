...
Each exposed device is a **config subentry** (`subentry_type="device"`, constants in `const.py`); its `data` is the device-config dict plus two metadata keys: `SUBENTRY_DATA_PLATFORM` (`"platform"`) naming the platform bucket (a former `options[<config_key>]` value, e.g. `switches`) and `SUBENTRY_DATA_AUTO_TITLE` (`"auto_title"`) holding the title as last derived from TapHome. Core-level config (zones, labels, webhook, enabled attributes) stays in `entry.options`. **`subentry.py` is the single source of truth for the subentry shape** — its unique-id format (`"{platform}:{id}"`), construction (`build_device_subentry[_data]`), iteration (`iter_device_subentries`) and extraction (`subentry_platform`, `device_config_from_subentry` — strips both metadata keys) helpers are reused by every consumer; do not hand-build subentry dicts elsewhere.

- Subentry **titles follow TapHome**: `config_flow.py:device_subentry_title` is the single title formula (`id, description-or-name, zone, category`, plus the value label for per-value sensors); `__init__.py:_async_sync_subentry_titles` re-applies it on every setup while the stored `auto_title` still equals the title, so a title the user renamed in HA is never overwritten. It runs before the update listener is registered, so its writes do not trigger a reload. (`_device_display_label` is the separate name-based label used only in the device-picker dropdown.)
