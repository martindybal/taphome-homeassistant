...
- **Number entities no longer cap at 100**: the min/max a variable reports in TapHome are applied, and when it reports none Home Assistant keeps its own defaults instead of a hard-coded 0–100 range (values above 100 previously failed to set). A number device's options now also offer optional **Minimum value / Maximum value / Step** overrides.
...
- **Device subentry titles now follow the TapHome device**: changing a device's description, zone or category in the TapHome app updates the subentry title on the next reload — unless you renamed the subentry in Home Assistant, in which case your name is kept. All subentries are titled consistently as `<id> - <description>, <zone>, <category>`.
