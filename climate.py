...
class TapHomeClimateConfig(TapHomeEntityConfig):
    ...
    def _backwards_compatibility(device_config):
        ...
        if isinstance(device_config, dict):
            ...
            if "heating_cooling_mode_id" in device_config:
                device_config["hvac_mode_id"] = device_config["heating_cooling_mode_id"]

            if "heating_switch_id" in device_config:
                device_config["hvac_switch_id"] = device_config["heating_switch_id"]
                device_config["hvac_mode"] = HVACMode.HEAT

            if "cooling_switch_id" in device_config:
                device_config["hvac_switch_id"] = device_config["cooling_switch_id"]
                device_config["hvac_mode"] = HVACMode.COOL
...
class HvacController(ABC):
    ...
    def _map_hvac_mode(self, index: int | None) -> HVACMode | None:
        return None if index is None else self.MODES_BY_INDEX.get(index)

    def _map_hvac_mode_to_index(self, hvac_mode: HVACMode | None) -> int | None:
        return None if hvac_mode is None else self.INDEX_BY_MODE.get(hvac_mode)

    def _map_hvac_action(self, index: int | None) -> HVACAction | None:
        if index is None:
            return None
...
class SwitchHvacControllerBase(HvacController, ABC):
    ...
    def _get_hvac_action(self, is_on: bool) -> HVACAction | None:
        if self._hvac_action_device is None:
            return None
        if is_on:
            return self._map_hvac_action(self._hvac_action_device.state.selected_value)
...
class DynamicSwitchHvacController(SwitchHvacControllerBase):
    ...
    def _hvac_mode_when_on(self) -> HVACMode | None:
        """Return the HVAC mode when the switch is on."""
        return self._map_hvac_mode(self._hvac_mode_device.state.selected_value)
...
class EmptyHvacController(HvacController):
    ...
    def __hvac_action_device_changed(
        ...
        self.hvac_action.value = self._map_hvac_action(new_state.selected_value)
...
class ModeHvacController(EmptyHvacController):
    ...
    def __init__(
        ...
        self.hvac_modes.value = list(self._map_hvac_modes())
    ...
    def _on_hvac_mode_changed(
        ...
        self.hvac_mode.value = self._map_hvac_mode(new_state.selected_value)

    def _map_hvac_modes(self) -> Generator[HVACMode]:
        for value in self._hvac_mode_device.allowed_values:
            mode = self._map_hvac_mode(value.value)
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode):
        ...
        hvac_mode_index = self._map_hvac_mode_to_index(hvac_mode)
        if hvac_mode_index is not None:
            await self._hvac_mode_device.async_select_value(hvac_mode_index)
...
class TapHomeRangeClimate(TapHomeClimateBase):
    ...
    def __init__(
        ...
        if (
            ...
            raise ValueError(
                "Both range_low_thermostat_id and range_high_thermostat_id must "
                "be set for TapHomeRangeClimate."
            )
...
def _create_climate_entity(
    ...
    raise ValueError(
        ...
        "Both range_high_thermostat_id and range_low_thermostat_id must be set "
