...
class DeviceState:
    ...
    def __post_init__(self) -> None:
        ...
        self.apply_changes(self._device_values, True)
    ...
    def apply_changes(
        self, values: dict[ValueType, float], initial: bool = False
    ) -> None:
        """Apply changes to the device state based on provided values."""
        updated = self._device_values != values
        if initial or updated:
            ...
            # Mypy don't recognize Self as a type here.
            self._changed(old_state, self)  # type: ignore[arg-type]
    ...
    def get_device_enum_value(
        self, enum_type: type[EnumT], value_type: ValueType
    ) -> EnumT | None:
...
class Device(DeviceMetadata, Generic[StateT]):
    ...
    async def async_set_device_values(
        ...
        if result is None:
            ...
            return
        self.ensure_values_changed(result)

        if self._connection_type.value != ApiConnectionType.LOCAL_PUSH:
            updated_values = await self._api.async_get_device_values(self.id)
            if updated_values is not None:
                self.state.apply_changes(updated_values.values)

    def ensure_values_changed(self, result: SetDeviceValueResponse) -> None:
