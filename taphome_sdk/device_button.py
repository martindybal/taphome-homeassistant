...
class ButtonState(DeviceState):
    ...
    def device_values_changed(self, initial: bool) -> None:
        ...
        # suppress_warning because Modbus buttons can have 0 value which is undefined
        self.action = self.get_device_enum_value(
            ButtonAction, ValueType.BUTTON_PRESSED, True
        )
        self.is_held = self.get_device_bool_value(ValueType.BUTTON_HELD_STATE)
...
class ButtonDevice(Device[ButtonState]):
    ...
    clicked: Event[ButtonAction] = field(default_factory=Event[ButtonAction])
    ...
    def _on_state_change(self, _: ButtonState | None, state: ButtonState) -> None:
        ...
        if state.is_held and state.action:
