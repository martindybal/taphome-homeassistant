...
@dataclass(slots=True, frozen=True)
class AllDevicesValuesResponse:
    """Response from getAllDevicesValues endpoint."""
    ...
    @classmethod
    def from_dict(cls, data: dict) -> AllDevicesValuesResponse:
        """Instantiate ``AllDevicesValues`` from a dictionary."""
        return AllDevicesValuesResponse(
            devices={
...
class TapHomeApi:
    ...
    async def async_get_all_devices_values(self) -> AllDevicesValuesResponse | None:
        """Return current values for all devices."""
        return await self.taphome_http_client.async_api_get(
            AllDevicesValuesResponse, "getAllDevicesValues"
        )
