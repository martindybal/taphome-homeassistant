...
class TapHomeHub:
    ...
    connection_state = ObservableValue(HubConnectionState.DISCONNECTED)
    ...
    async def async_handle_webhook(self, request: Request) -> None:
        ...
        if self.connection_state.value == HubConnectionState.CONNECTED:
            ...
            changed_values = AllDevicesValuesResponse.from_dict(json)
    ...
    def _empty_response(self) -> NoReturn:
        raise ValueError("Empty response from TapHome API")
...
class TapHomeHubFactory:
    ...
    async def async_connect(api_url: str, token: str) -> TapHomeHub:
        ...
        if hub.connection_type == ApiConnectionType.CLOUD:
            _LOGGER.warning(
